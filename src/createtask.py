...
def dataset_fn(split, shuffle_files=False):
    ...
    ds = tf.data.TextLineDataset([os.path.join("gs://"+nq_tsv_path["bucket"],nq_tsv_path[split],str(filename)) for filename in client.list_blobs(nq_tsv_path["bucket"], prefix=nq_tsv_path[split]) if str(filename).startswith(f'{nq_tsv_path["taskname"].replace("ccp", "context-ps")}-{split}')], compression_type=nq_tsv_path["compression"]).filter(lambda line:tf.not_equal(tf.strings.length(line),0))
